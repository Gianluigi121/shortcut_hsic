...
"""Runs the full correlation sweep for the chexpert experiment."""
import functools
import itertools
import subprocess
...
def runner(config, base_dir, checkpoint_dir, slurm_save_dir, overwrite,
	...
	try:
		subprocess.call(f'python -m chexpert_support_device.main {flags} > {model_dir}/log.log 2>&1',
			shell=True)
		# subprocess.call('python -m chexpert.main %s' % flags, shell=True)
		# print(f'python -m chexpert.main {flags} > {model_dir}/log.log 2>&1')
	finally:
