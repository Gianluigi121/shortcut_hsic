...
def configure_hsic_model(v_mode, v_dim, weighted, batch_size):
...
def configure_baseline(v_mode, v_dim, weighted, batch_size):
	"""Creates hyperparameters for correlations experiment for SLABS model.
...
def get_sweep(model, v_mode, v_dim, batch_size):
	...
	if model == 'unweighted_baseline':
		return configure_baseline(v_mode=v_mode, v_dim=v_dim,
		 weighted='False', batch_size=batch_size)

	if model == 'weighted_baseline':
		return configure_baseline(v_mode=v_mode, v_dim=v_dim, weighted='True',
			batch_size=batch_size)

	if model == 'weighted_bal_baseline':
		return configure_baseline(v_mode=v_mode, v_dim=v_dim, weighted='True_bal',
			batch_size=batch_size)


	if model == 'unweighted_hsic':
		return configure_hsic_model(v_mode=v_mode, v_dim=v_dim, weighted='False',
			batch_size=batch_size)
