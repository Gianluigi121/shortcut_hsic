...
def create_save_chexpert_lists(experiment_directory, p_train=0.9, random_seed=None):
    ...
    df = pd.read_csv(MAIN_DIR+'final_pen_nofinding_cohort.csv')
...
def create_dataset(csv_dir, params, is_train=False):
...
def load_ds_from_csv(experiment_directory, skew_train, params):
    ...
    unskew_test_ds = create_dataset(unskew_test_csv_dir, params)

    skew_test_csv_dir = f'{experiment_directory}/skew_test.csv'
    skew_test_ds = create_dataset(skew_test_csv_dir, params)
