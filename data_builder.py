...
def fix_marginal(df, y0_probability, rng):
    y0_group = df.index[(df.y1 == 0)]
    y1_group = df.index[(df.y1 == 1)]
...
def create_save_chexpert_lists(experiment_directory, p_train=0.9, random_seed=None):
    ...
    df = pd.read_csv(MAIN_DIR+'final_nofinding_cohort.csv')
...
def load_ds_from_csv(experiment_directory, skew_train, params):
    ...
    train_ds = create_dataset(train_csv_dir, params, test=False)

    valid_csv_dir = f'{experiment_directory}/{skew_str}_valid.csv'
    valid_ds = create_dataset(valid_csv_dir, params, test=False)

    test_ds_dict = {}
    for pskew in [0.5]:
        test_csv_dir = f'{experiment_directory}/skew_test_{pskew}.csv'
        test_ds_dict[pskew] = create_dataset(test_csv_dir, params, test=True)
...
def create_dataset(csv_dir, params, test=True):
    df = pd.read_csv(csv_dir)
    df.masked_imgs = '/nfs/turbo/coe-rbg/zhengji/age_mask/'+df.masked_imgs
    ...
    if (test):
        ds = ds.map(map_to_image_label).batch(1e5)
    else:
